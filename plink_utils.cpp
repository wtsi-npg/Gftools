...
bool plink_utils::ind_cmp(std::vector<gftools::individual> ind1, std::vector<gftools::individual> ind2)
{
    ...
    for(size_t i = 0; i < ind1.size(); i++) {
        if (ind1[i].name.compare(ind2[i].name)) return false;
        if (ind1[i].family.compare(ind2[i].family)) return false;
    }
    return true;
}
...
{
    if (snp1.size() != snp2.size()) return false;
    for(size_t i = 0; i < snp1.size(); i++) {
        if (snp1[i].name.compare(snp2[i].name)) return false;
    }
    return true;
}
